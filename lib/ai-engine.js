...
function createAiEngine(env = process.env) {
  const llamaApiKey = String(env.LLAMA_API_KEY || "").trim();
  // Nemotron authenticates with its own key; fall back to the Llama key so
  // existing single-key setups keep working when NVIDIA_API_KEY is unset.
  const nemotronApiKey = String(env.NVIDIA_API_KEY || env.LLAMA_API_KEY || "").trim();
  const configured = Boolean(llamaApiKey || nemotronApiKey);
  ...
  const apiKeyForModel = (model) => (model === nemotronModel ? nemotronApiKey : llamaApiKey);
  const keyNameForModel = (model) => (model === nemotronModel ? "NVIDIA_API_KEY" : "LLAMA_API_KEY");
  ...
  const state = {
    ...
    apiKeys: {
      llama: Boolean(llamaApiKey),
      nemotron: Boolean(String(env.NVIDIA_API_KEY || "").trim()),
    },
  ...
  async function callModel(model, prompt) {
    ...
    if (!apiKey) throw new Error(`${keyNameForModel(model)} is not configured`);
  ...
  async function evaluate(input) {
    ...
    const modelChoice = normalizeModelChoice(input.model, mode === "live" ? state.liveModel : state.paperModel);
    const proposedSide = normalizedSide(input.signal?.side || input.side);
    const marketId = String(input.marketId || "unknown");
    const cacheKey = `${venue}:${mode}:${modelChoice}:${marketId}:${proposedSide}:${Number(input.price || 0).toFixed(2)}`;
    ...
    if (!state.configured) {
      const decision = { ...base, reason: "No AI model API key (LLAMA_API_KEY or NVIDIA_API_KEY) is configured; trade blocked." };
      state.decisions[venue] = decision;
      return decision;
    }
    if (!proposedSide) {
